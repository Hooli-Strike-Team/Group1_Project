            ...
            keypad[i - 1].addEventListener('click', function (e) {
                ...
                if (element) {
                    ...
                    if (notes_mode) {
                        ...
                        let move_result = game1.make_move(row, col, this.getAttribute('data-digit'), timer, difficulty_check);
                        game1.clear_notes(element);
                        console.log(game1.board);

                        // Handle mistakes when mistakes_mode is active
                        if (mistakes_mode) {
                            game1.board_state(sudoku_squares, "invalid");
                            if (!move_result) {
                                // Increment mistakes_count and update the counter
                                mistakes_count++;
                                document.getElementById('strike-counter').innerHTML = mistakes_count;

                                // Check if 10 mistakes have been reached and display the modal
                                if (mistakes_count == 10) {
                                    rendermodal();
                                }
                            }
                        }
